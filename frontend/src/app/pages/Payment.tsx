...
export function Payment() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <p className="text-red-800">Parcarea nu a fost găsită</p>
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Plata a eșuat. Încearcă din nou.');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <h1 className="font-bold mb-2">Cumpără tichet de parcare</h1>
        <p className="text-gray-600">Selectează durata și finalizează plata</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="font-semibold mb-4">Detalii parcare</h2>
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">${lot.hourlyRate}/oră</p>
            <p className="text-sm text-green-600 mt-1">{availableCount} locuri disponibile</p>
          </div>
        </div>

        <div className="pt-4 border-t border-gray-200">
          <label className="block text-sm mb-2">Durată parcare</label>
      ...
      {/* Select Vehicle */}
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="font-semibold mb-4">Selectează vehiculul</h2>
        {vehicles.length === 0 ? (
          <div className="text-center py-6 bg-yellow-50 rounded-lg border border-yellow-200">
            <Car className="w-10 h-10 text-yellow-500 mx-auto mb-3" />
            <p className="text-gray-700 font-medium mb-1">Niciun vehicul adăugat</p>
            <p className="text-sm text-gray-500 mb-4">Ai nevoie de cel puțin un vehicul pentru a cumpăra un tichet de parcare.</p>
            <button
            ...
            <p className="text-xs text-gray-400 mt-3">Vei fi adus înapoi după adăugare.</p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Categorie {car.vehicleCategory}{car.electric ? ' • Electric' : ''}</p>
                      {activeVehicleIds.has(car.id) && (
                        <span className="inline-flex items-center gap-1 mt-1 text-xs font-medium text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full">
                          <AlertTriangle className="w-3 h-3" /> Sesiune activă
                        </span>
                      )}
                    </div>
                  </div>
      ...
      {/* Payment Method */}
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="font-semibold mb-4">Metodă de plată</h2>
        <div className="space-y-3">
          <button
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">Portofel digital</span>
              </div>
      ...
      {/* Summary */}
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="font-semibold mb-4">Rezumat</h2>
        <div className="space-y-3">
          <div className="flex items-center justify-between py-2 border-b border-gray-100">
            <span className="text-gray-600">Durată</span>
            <span className="font-medium">{hours} ore</span>
          </div>
          <div className="flex items-center justify-between py-2 border-b border-gray-100">
            <span className="text-gray-600">Tarif</span>
            <span className="font-medium">${lot.hourlyRate}/oră</span>
          </div>
          <div className="flex items-center justify-between py-2 border-b border-gray-100">
      ...
      {selectedCarHasActiveTicket && (
        <div className="mb-4 p-4 bg-amber-50 border border-amber-300 rounded-lg flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-amber-800">Sesiune activă detectată</p>
            <p className="text-sm text-amber-700 mt-0.5">Acest vehicul are deja o sesiune de parcare activă. Te rugăm să o închei pe pagina Tichete înainte de a cumpăra un nou tichet.</p>
          </div>
        </div>
      )}
      ...
      >
        {processing ? 'Se procesează...' : `Plătește $${totalPrice.toFixed(2)} și obține tichet`}
      </button>
    </div>
  );
}
